...
DATABASE_HOST = "hmtowdb34.sharedtcs.net"

DATABASE_USER = "enm"

#DATABASE_PW = ""
DATABASE_PW = "yJavcvrDURd5JzHb"

#DATABASE_NAME = "test"
DATABASE_NAME = "rantools"

DATABASE_TABLE = "alarmsenm"
DATABASE_TABLE_TEMP = "AlarmsEnm_temp"
...
def insert_fm_response_to_database(response_lines):

    # Connect to the MySQL DB
    logger.debug("Connecting to MySQL database host '{0}' with user '{1}' on table '{2}'".format(DATABASE_HOST, DATABASE_USER, DATABASE_NAME))
    connection = MySQLdb.connect(host=DATABASE_HOST, user=DATABASE_USER, passwd=DATABASE_PW, db=DATABASE_NAME)

    with connection:
        cursor = connection.cursor()

        # Delete old temp table if still existing and create new one
        with warnings.catch_warnings():
            warnings.simplefilter('ignore')
            cursor.execute("DROP TABLE IF EXISTS {0}".format(DATABASE_TABLE_TEMP))
            cursor.execute("CREATE TABLE {0} LIKE {1}".format(DATABASE_TABLE_TEMP, DATABASE_TABLE))
        
        # Skip the first line as this contains the column headers
        for line in response_lines[1:-2]:

            # Split the line string into a list using semi-colon as the delimiter 
            line = line.split(";")
            ...
            event_time = line [3]
            #fdn = line [4]
            object_of_reference = line[5]

            # The "node" and "exchange" values can be found within the object of reference line
            # Split the line into a list using comma delimiter and find the value(s) after the equals sign (exchange first, then node)
            exchange = object_of_reference.split(",")[1].split("=")[1]
            node = object_of_reference.split(",")[2].split("=")[1]

            present_severity = line[6]
            problem_detail = line[7]
            problem_text = line[8]
            specific_problem = line[9]
            
            # Convert the date time to desired format - Example: "18/11/2015 01:58:37.000" > "18.11.2015 01:58:37" 
            datetime_obj = datetime.datetime.strptime(event_time, '%d/%m/%Y %H:%M:%S.%f')         
            formatted_event_time = datetime.date.strftime(datetime_obj, "%Y.%m.%d %H:%M:%S")

            # Set the technology based on the Exchange value
            if "LRAN" in exchange:
                technology = "LTE"
            elif "RNC" in exchange:
                technology = "UMTS"
            else:
                technology = ""

            # Remove 'SubNetwork=' string from FDN value
            object_of_reference = re.sub(r"SubNetwork=.\w*,", r"", object_of_reference.rstrip())

            # Build the MySQL query
            query = "INSERT  INTO {0}{1} VALUES{2}".format(DATABASE_TABLE_TEMP, "(Node,Exchange,Severity,ProblemText,ProblemDetail,SpecificProblem,EventTime,Technology,Fdn,AlarmNumber,AckOperator,AlarmState)", \
                (node,exchange,present_severity,problem_text,problem_detail,specific_problem,formatted_event_time,technology, object_of_reference, alarm_number, ack_operator, alarm_state))

            logger.debug("Executing MySQL query '{0}'".format(query))
            cursor.execute(query)

        # Delete old table and replace it with the temp table
        cursor.execute("DROP TABLE {0}".format(DATABASE_TABLE))
        cursor.execute("ALTER TABLE {0} RENAME TO {1}".format(DATABASE_TABLE_TEMP, DATABASE_TABLE))
...
def main():
    ...
    try:
        ...
        # Write the alarm data in the response to the MySQL database
        insert_fm_response_to_database(response_lines)
