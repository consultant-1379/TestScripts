...
then
	...
	then
		...
		S4_INSTALL_SCRIPT=`ssh -q -t -o StrictHostKeyChecking=no -o ConnectTimeout=5 -o BatchMode=yes  root@${MSIP} "touch ${LOG_FILE};chmod 755 /var/tmp/s4/s4_install_rpm_lms.sh; /var/tmp/s4/s4_install_rpm_lms.sh ${DEPLOYMENT_ID} ${ORIGNAL_RPM} ${JIRA_TICKET_ID} ${install_type} ${LIST_OF_SERVICE_GROUPS_TO_OFFLINE} ${TIMESTAMP} ${LOG_FILE} | tee ${LOG_FILE}"`
		#echo "{\"LEVEL\":\"SUCCESS\",\"MESSAGE\":\"RPM installation has been started on ${LIST_OF_SERVICE_GROUPS_TO_OFFLINE}\"}";
		exit;
		
	else
			echo "{\"LEVEL\":\"ERROR\",\"MESSAGE\":\"Could not rollback RPM - s4_install_rpm_lms.sh is missing on LMS\"}";
			exit;
