...
NE_COUNT=`${CLI_APP} "cmedit get *ERBS* InventorySupervision -ne=ERBS -cn" | grep -v found | awk '{print $1}'`
...
NE_LIST=`${CLI_APP} "cmedit get * InventorySupervision -ne=ERBS" | grep FDN | grep ieatnetsimv | cut -d'=' -f2 | cut -d',' -f1 | sort -R | tail -${LIMIT}| sed 's/$/;/g'`
...
function test_KPI(){
   ...
   while [ "${SYNC}" -lt "${LIMIT}" ] && [ "${LOOP}" -lt 20 ]
   do
      let LOOP=LOOP+1
      GET_NUM_SYNC_NODES
      INTERVAL=$(date +%s);
   ...
   echo "*************************************************************************************"
   echo "SHM INVENTORY SYNC HAS BEEN SUCCESSFUL FOR ${SYNC} nodes out of ${LIMIT} NODES in ${IT}"
   echo "====================================================================================="
