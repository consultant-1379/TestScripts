...
mount_ddp_to_lms(){
    ...
    [[ -z $(mount | egrep ddp2:) ]] && mount ddp2:/data/stats /net/ddp2/data/stats || echo "already mounted"
...
then
        ...
        $ECHO ""
        $ECHO "Usage   : # /ericsson/enm/dumps/.scripts/.dps_events_flow_invocation_CustomerExtended.sh [-h|--h|-help|--help]:  --->  Prints this help/usage information for this script"
        $ECHO ""
        $ECHO "Usage   : # /ericsson/enm/dumps/.scripts/.dps_events_flow_invocation_CustomerExtended.sh DDP SITE DAY1 DAY2 MONTH YEAR"
        $ECHO " "
        $ECHO "Example  :"
        $ECHO -e "\t# /ericsson/enm/dumps/.scripts/.dps_events_flow_invocation_CustomerExtended.sh ddp2 Verizon_Azusa_azusca21enm-e-ec-c7kx-ms1_Core_LTE 09 10 03 19 "
        $ECHO ""
        $ECHO "          Note: This takes some time but but returns a significant volume of Data with respect to Resource Usage, DPS Events, Network managed etc"
        $ECHO ""
        exit
fi
...
if [ -f $DDP/$SITE/data/${i}${DDP_MONTH}${DDP_YR}/TOR//sw_inventory/LITP2_deployment_description.gz ]; then
        ...
        find $DDP/$SITE/data/${i}${DDP_MONTH}${DDP_YR}/ -type f |grep engine_A.log|xargs cat|egrep --colour 'VCS ERROR|VCS WARNING'|grep -oP "VCS.*"|sort |uniq -c|grep -oP "VCS.*"|egrep -v "monitor procedure did not complete within the expected time|because online did not complete within the expected time|consistently failed to determine the resource status within the expected time"|egrep --colour "ERROR"|sed "s/^/\t\t/g";
