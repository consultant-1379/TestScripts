...
def has_passwordless_access(user, server):
    rc = run_cmd("ssh -vvv -o ConnectTimeout=5 -o PreferredAuthentications=publickey -o BatchMode=yes {0}@{1} 'hostname'".format(user, server), "Public key-based SSH connection test failed")
...
def copy_ssh_key_to_server(server, user, passwd, keyfile=None):
    ...
    run_cmd("ssh-keygen -R {0} > /dev/null 2>&1".format(server), "Could not remove current host key from our known_hosts file")
    run_cmd("ssh-keyscan -H {0} >> ~/.ssh/known_hosts 2> /dev/null".format(server), "Could not add host key to our known_hosts file")
